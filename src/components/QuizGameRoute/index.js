...
const QuizGameRoute = () => {
  ...
  const renderFailureView = () => (
    <div className="quiz-failure-main-container">
      <div className="quiz-failure-container">
        <img
          ...
          return options.map((option, index) => {
            ...
            return (
              <li key={option.id} className={`option-container ${optionClass}`}>
                ...
                >
                  <span className="options-code">{`${String.fromCharCode(
                    65 + index,
                  )}. `}</span>
                  {option.text}
                  {showIcon && (
        ...
        <div className="quiz-page-content">
          <div className="question-time-container">
            <div className="number-container">
              <p className="question-text-para">Question</p>
              <p className="question-count">
                {activeQuestionIndex + 1}/{quizQuestions.length}
              </p>
            </div>
            <div className="time-container">
              <p className="timer-sec-count">{timer}</p>
            </div>
          </div>
